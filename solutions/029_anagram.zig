...
pub fn detectAnagrams(
    ...
    const reference = computeSignature(word);

    for (candidates) |cand| {
        if ((stringIsDifferent(word, cand)) and mem.eql(u8, &reference, &computeSignature(cand))) {
            try set.insert(cand);
        }
    }
...
pub fn stringIsDifferent(string1: []const u8, string2: []const u8) bool {
    if (string1.len != string2.len) return true;

    for (string1, string2) |c1, c2| {
        if (std.ascii.toLower(c1) != std.ascii.toLower(c2)) return true;
    }

    return false;
}

pub fn computeSignature(word: []const u8) [26]u8 {
    var signature: [26]u8 = undefined;
    for (0..26) |i| signature[i] = 0;
    for (word) |letter| {
        const i: usize = @as(usize, std.ascii.toLower(letter) - 'a');
        signature[i] += 1;
    }
    return signature;
}
