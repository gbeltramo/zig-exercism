...
pub fn format(allocator: mem.Allocator, name: []const u8, number: u10) ![]u8 {
    ...
    const number_as_string = try std.fmt.bufPrint(buf, "{d}", .{number});

    const last_digit = number_as_string[number_as_string.len - 1];
    var ordinal_ending = switch (last_digit) {
        '1' => "st",
        '2' => "nd",
        '3' => "rd",
        else => "th",
    ...
    if (number_as_string.len >= 2) {
        if (number_as_string[number_as_string.len - 2] == '1') {
            ordinal_ending = "th";
        }
    }
    const buf_sentence = try allocator.alloc(u8, name.len + number_as_string.len + 2 + 50);

    const sentence = try std.fmt.bufPrint(buf_sentence, "{s}, you are the {d}{s} customer we serve today. Thank you!", .{ name, number, ordinal_ending });

    return sentence;
}

fn testFormat(allocator: std.mem.Allocator, expected: []const u8, name: []const u8, number: u10) !void {
    const actual = try format(allocator, name, number);
