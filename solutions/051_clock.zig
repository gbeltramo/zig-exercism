...
pub const Clock = struct {
    ...
    pub fn init(hour: i32, minute: i32) Clock {
        ...
        while (total_minutes < 0) total_minutes += MINUTES_IN_A_DAY;
        while (total_minutes >= MINUTES_IN_A_DAY) total_minutes -= MINUTES_IN_A_DAY;
        const h: i32 = @divFloor(total_minutes, 60);
        const m = @mod(total_minutes, 60);
        return Clock{ .hour = h, .minute = m };
    }

    /// Moves the clock forwards by `minutes` minutes.
    pub fn advance(self: *Clock, minutes: i32) void {
        const new_clock = Clock.init(self.hour, self.minute + minutes);
        self.hour = new_clock.hour;
        self.minute = new_clock.minute;
    }
    ...
    pub fn toString(self: Clock) [5]u8 {
        const new_clock = Clock{ .hour = self.hour, .minute = self.minute };
        var buffer: [5]u8 = undefined;

        _ = std.fmt.bufPrint(&buffer, "{d:0>2}:{d:0>2}", .{
            @as(u8, @intCast(new_clock.hour)),
            @as(u8, @intCast(new_clock.minute)),
        }) catch unreachable;
        return buffer;
    }
    ...
    pub fn eql(self: Clock, other: Clock) bool {
        const c1 = Clock.init(self.hour, self.minute);
        const c2 = Clock.init(other.hour, other.minute);
        return (c1.hour == c2.hour) and (c1.minute == c2.minute);
    }
};
